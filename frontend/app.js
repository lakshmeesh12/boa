...
// NOTE: deliberately no client-side validation -- the whole point of the
...
async function openInstantLimitIncreaseModal() {
  ...
  if (lbl) { lbl.textContent = 'Loading active card...'; }
  ...
  try {
    ...
    if (lbl) {
      lbl.textContent = 'Card ****' + last4 + (limit != null ? '   Current limit $' + Number(limit).toLocaleString() : '');
    }
  } catch (e) {
...
async function submitInstantLimitIncrease() {
  ...
  try {
    ...
    if (resp.ok) {
      if (res) {
        res.style.display = '';
        res.textContent = 'Approved. New credit limit: $' + Number(data.new_limit).toLocaleString();
      }
