...
// Uses an in-app modal so the UI agent can drive it via real DOM elements.
// NOTE: deliberately no client-side validation â€” the whole point of the
// demo is that AQE detects the missing server-side validation on
// POST /credit-cards/{id}/limit-increase (negative amounts accepted, etc.).
// Adding JS-side guards would mask the bug from the API/UI test plan.
var _ilimSelectedCardId = null;

async function openInstantLimitIncreaseModal() {
  var btn = document.getElementById('btn-instant-limit-increase');
  var lbl = document.getElementById('ilim-card-label');
  var err = document.getElementById('ilim-error');
  var res = document.getElementById('ilim-result');
  var amt = document.getElementById('ilim-amount');
  if (err) { err.style.display = 'none'; err.textContent = ''; }
  if (res) { res.style.display = 'none'; res.textContent = ''; }
  if (amt) { amt.value = '1000'; }
  if (lbl) { lbl.textContent = 'Loading active cardâ€¦'; }
  if (btn) { btn.disabled = true; }
  if (typeof openModal === 'function') openModal('modal-instant-limit-increase');
  try {
    var listResp = await fetch('/api/v1/credit-cards?status=ACTIVE&limit=1');
    var listData = await listResp.json();
    if (!listData.cards || !listData.cards.length) {
      if (lbl) { lbl.textContent = 'No active cards available.'; }
      if (err) { err.style.display = ''; err.textContent = 'No active cards found on this account.'; }
      _ilimSelectedCardId = null;
      return;
    }
    _ilimSelectedCardId = listData.cards[0]._id || listData.cards[0].id;
    var last4 = (listData.cards[0].card_number_last4 || listData.cards[0].last4 || '----');
    var limit = listData.cards[0].credit_limit;
    if (lbl) {
      lbl.textContent = 'Card â€¢â€¢â€¢â€¢ ' + last4 + (limit != null ? '  â€¢  Current limit $' + Number(limit).toLocaleString() : '');
    }
  } catch (e) {
    if (lbl) { lbl.textContent = 'Failed to load card.'; }
    if (err) { err.style.display = ''; err.textContent = 'Error loading card: ' + e.message; }
  } finally {
    if (btn) { btn.disabled = false; }
  }
}
window.openInstantLimitIncreaseModal = openInstantLimitIncreaseModal;

async function submitInstantLimitIncrease() {
  var input = document.getElementById('ilim-amount');
  var err = document.getElementById('ilim-error');
  var res = document.getElementById('ilim-result');
  var submitBtn = document.getElementById('ilim-submit-btn');
  if (err) { err.style.display = 'none'; err.textContent = ''; }
  if (res) { res.style.display = 'none'; res.textContent = ''; }

  if (!_ilimSelectedCardId) {
    if (err) { err.style.display = ''; err.textContent = 'No active card selected.'; }
    return;
  }
  var raw = input ? String(input.value || '').trim() : '';
  var amount = Number(raw);

  if (submitBtn) { submitBtn.disabled = true; submitBtn.textContent = 'Processingâ€¦'; }
  try {
    var resp = await fetch('/api/v1/credit-cards/' + _ilimSelectedCardId + '/limit-increase', {
      method: 'POST',
    ...
    if (resp.ok) {
      if (res) {
        res.style.display = '';
        res.textContent = 'âœ“ Approved. New credit limit: $' + Number(data.new_limit).toLocaleString();
      }
      if (typeof toast === 'function') toast('Credit limit increased to $' + Number(data.new_limit).toLocaleString());
    } else {
      if (err) {
        err.style.display = '';
        err.textContent = 'Request failed: ' + (data.detail || resp.status);
      }
    }
  } catch (e) {
    if (err) { err.style.display = ''; err.textContent = 'Error: ' + e.message; }
...
window.submitInstantLimitIncrease = submitInstantLimitIncrease;
window.instantLimitIncrease = openInstantLimitIncreaseModal;
