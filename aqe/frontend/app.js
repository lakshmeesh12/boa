...
// Stores the ChangeContext that triggered the current change-driven session,
// so the rp-plan header card and renderPlan() can show file count + risk.
let _changeCtx=null;
...
async function startChangeDrivenSession(planMode){
  ...
  try{
    // Capture the change context BEFORE creating the session so we can show it
    // in the rp-plan header card immediately.
    try { _changeCtx = await apiFetch("/changes/since-baseline"); } catch(_) { _changeCtx=null; }
    ...
    // Hide the manual-config left sidebar for change-driven sessions
    const grid=$("run-tests-grid"); if(grid) grid.classList.add("change-driven-mode");
    _sid=sess.id;
    _sidPlanMode = planMode;
...
let _sidPlanMode=null;            // 'smart'|'full'|'new_only' (change-driven) or null
let _lByCategory={};              // {Vulnerability: {pass:0,fail:0,err:0,total:0}, ...}
...
// Category ordering for plan grouping — most critical first
const _CATEGORY_ORDER = ["Vulnerability","Security","Performance","Header","Unit","UI","API","Integration","Functional"];

function _renderChangeContextCard(){
  const card=$("plan-ctx-card");
  if(!card) return;
  if(!_changeCtx || !_changeCtx.change_set || _changeCtx.change_set.is_empty){
    card.style.display="none";
    return;
  }
  card.style.display="";
  const cs=_changeCtx.change_set, a=_changeCtx.analysis||{};
  // Hide the legacy AI Plan Summary box when we have a change context card (avoid duplication)
  const legacy=$("plan-summary-wrap"); if(legacy) legacy.style.display="none";

  setText("plan-ctx-summary", a.summary || "Change-driven plan triggered by your latest commit.");
  setText("plan-ctx-files", `${cs.file_count} file${cs.file_count===1?"":"s"} changed (+${cs.total_additions}/-${cs.total_deletions})`);

  const riskCls={low:"p-slate",medium:"p-amber",high:"p-red",critical:"p-red"};
  const rp=$("plan-ctx-risk");
  if(rp){
    rp.className="pill "+(riskCls[a.risk_level]||"p-slate");
    rp.textContent=(a.risk_level||"low").toUpperCase()+" RISK";
  }
  const link=$("plan-ctx-commit");
  if(link){
    if(a.github_commit_url){link.style.display=""; link.href=a.github_commit_url;}
    else{link.style.display="none";}
  }
}

window.togglePlanCtxSummary=function(){
  const el=$("plan-ctx-summary"), tog=$("plan-ctx-toggle");
  if(!el) return;
  const isExp=el.classList.toggle("expanded");
  if(tog) tog.textContent=isExp?"Show less ▴":"Show more ▾";
};

function _categoryOf(item){
  const tc=item.test_case||item;
  return tc.category || _legacyTypeToCategory(tc.test_type) || "Functional";
}

function _legacyTypeToCategory(t){
  if(t==="Security") return "Security";
  if(t==="EdgeCase") return "Functional";
  return "Functional";
}
...
function renderPlan(plan){
  ...
  // Either show change-context card OR legacy plan-summary box (mutually exclusive)
  if(_sidPlanMode && _changeCtx){
    _renderChangeContextCard();
  } else {
    const card=$("plan-ctx-card"); if(card) card.style.display="none";
    const legacy=$("plan-summary-wrap"); if(legacy) legacy.style.display="";
  ...
  items.forEach((item,i)=>{
    const c=_categoryOf(item);
    (byCat[c]=byCat[c]||[]).push({...item,_idx:i+1});
  });

  // Ordered list of categories present in plan
  const cats=_CATEGORY_ORDER.filter(c=>byCat[c]).concat(
    Object.keys(byCat).filter(c=>!_CATEGORY_ORDER.includes(c))
  );

  list.innerHTML=cats.map(cat=>{
    const arr=byCat[cat];
    const rows=arr.map(item=>_renderPlanItem(item,cat)).join("");
    return `<div class="plan-section-h"><span>${cat}</span><span class="sec-count">(${arr.length})</span></div>${rows}`;
  }).join("");
}

function _renderPlanItem(item,cat){
  const tc=item.test_case||item;
  const idx=item._idx||"";
  const name=tc.name||"—";
  const mod=tc.module||"—";
  const rationale=item.rationale||"";
  const triggered=Array.isArray(tc.triggered_by_files)?tc.triggered_by_files:[];
  const triggeredHtml=triggered.length
    ? `<div style="font-size:10px;color:#94a3b8;margin-top:2px;">triggered by: ${escapeHtml(triggered.slice(0,2).join(", "))}${triggered.length>2?` +${triggered.length-2}`:""}</div>`
    : "";
  const rationaleHtml=rationale
    ? `<div style="font-size:10.5px;color:#64748b;margin-top:2px;font-style:italic;">${escapeHtml(rationale)}</div>`
    : "";
  return `<div style="display:flex;align-items:flex-start;gap:8px;padding:8px 12px;background:#fff;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:4px;">
    <div style="width:20px;height:20px;border-radius:50%;background:#f1f5f9;color:#64748b;font-size:10px;font-weight:700;display:flex;align-items:center;justify-content:center;flex-shrink:0;margin-top:1px;">${idx}</div>
    <div style="flex:1;min-width:0;">
      <div style="font-size:12.5px;font-weight:600;color:#0f172a;">${escapeHtml(name)}</div>
      <div style="display:flex;align-items:center;gap:6px;margin-top:3px;flex-wrap:wrap;">
        <span style="font-size:11px;color:#64748b;">${escapeHtml(mod)}</span>
        <span class="cat-chip cat-${escapeHtml(cat)}">${escapeHtml(cat)}</span>
      </div>
      ${rationaleHtml}
      ${triggeredHtml}
    </div>
...
window.approveAndRun=async function(){
  ...
  _lPass=0;_lFail=0;_lErr=0;_lTotal=0;
  _lByCategory={};  // reset category counters
  resetExecUI();
  showRp("rp-exec");
  // CRITICAL: reconnect WS in exec mode so log + test_result + ui_frame events
  // route to the visible #agent-log / #test-out / #ui-live, not the hidden
  // planning panel. Without this, the right side of the screen stays blank.
  connectWs(_sid, false);
  _startT=new Date();startTimer();
  appendLog("t-info","[Engine] Session approved — execution starting…");
};
...
function drawUiFrame(b64){
  showUiStreamPanel();
  // First frame: hide the "Browser starting…" placeholder
  const ph=$("ui-live-placeholder"); if(ph && ph.style.display!=="none") ph.style.display="none";
  ...
  _uiFrameCount++;
  if(_uiFrameCount===1 || _uiFrameCount%6===0){
    if(!_uiFpsStart) _uiFpsStart=Date.now();
...
function handleWs(envelope, planMode){
  ...
  if(type==="log"){
    ...
    // Pre-show the live UI panel as soon as the UIAgent starts so the
    // "Browser starting…" placeholder is visible during the slow boot.
    if(!planMode && (data.agent==="UIAgent" || /UIAgent|UI \(Computer Use\)/.test(data.message||""))){
      showUiStreamPanel();
    }
  }else if(type==="test_result"){
    _lTotal++;
    const cat = data.category || "API";
    // Per-category counters
    const c = _lByCategory[cat] = _lByCategory[cat] || {pass:0,fail:0,err:0,total:0};
    c.total++;
    const tag = `[${(cat||"").toUpperCase().slice(0,4)}]`;
    if(data.status==="PASSED"){_lPass++; c.pass++; appendTerm("t-pass",`✓  ${tag} ${data.test_name}  (${data.duration_ms}ms)`);}
    else if(data.status==="FAILED"){_lFail++; c.fail++; appendTerm("t-fail",`✗  ${tag} ${data.test_name}  —  ${(data.error||"").slice(0,160)}`); }
    else{_lErr++; c.err++; appendTerm("t-warn",`⚠  ${tag} ${data.test_name}  —  ${(data.error||"").slice(0,160)}`);}
    updateExecProg();
...
function updateExecProg(){
  ...
  // Per-category mini-counter row
  const wrap=$("cat-counters"); if(!wrap) return;
  const cats=Object.keys(_lByCategory).sort();
  wrap.innerHTML=cats.map(c=>{
    const s=_lByCategory[c];
    return `<span class="cat-counter cat-${escapeHtml(c)}">${c}: <span class="pass-num">${s.pass}</span>/<span class="fail-num">${s.fail+s.err}</span>/${s.total}</span>`;
  }).join("");
}
...
window.resetToIdle=function(){
  ...
  _sidPlanMode=null; _lByCategory={}; _changeCtx=null;
  const sl=$("scripts-list");if(sl)sl.innerHTML="";
  // Restore the manual-config sidebar (remove change-driven-mode class)
  const grid=$("run-tests-grid"); if(grid) grid.classList.remove("change-driven-mode");
  showRp("rp-idle");loadIdleSessions();
};
